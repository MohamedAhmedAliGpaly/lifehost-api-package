// lifehost-api.c - helper binary for calling lifehost api classes
// Usage: lifehost-api ${API_METHOD} ${ADDITIONAL_FLAGS}
//        This executes
//          am broadcast com.lifehost.api/.TermuxApiReceiver --es socket_input ${INPUT_SOCKET}
...
_Noreturn void exec_am_broadcast(int argc, char** argv, char* input_address_string, char* output_address_string)
{
    ...
    child_argv[3] = "0";
    child_argv[4] = "-n";
    child_argv[5] = "com.lifehost.api/.LifehostApiReceiver";
    child_argv[6] = "--es";
...
_Noreturn void exec_callback(int fd)
{
    ...
    execl(PREFIX "/libexec/lifehost-callback", "lifehost-callback", fds, NULL);
    perror("execl(\"" PREFIX "/libexec/lifehost-callback\")");
    exit(1);
}
